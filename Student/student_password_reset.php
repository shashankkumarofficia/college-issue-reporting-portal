...
if (isset($_POST['update'])) {
    ...
    $select = ("select *from student where Password=$op");
    ...
    if ($row == true) {
        if ($newpassword == $confirmpassword) {
            $sql = mysqli_query($conn, "update student set Password='$newpassword' where Email='$user'");

            if ($sql == true) {
                echo "<script>alert('password changed')</script>";
                echo "<script>window.location.replace('student_password_reset.php')</script>";
            }
        } else {
